...
class C {
  ...
  add(b: C): C { return new C(this.re + b.re, this.im + b.im) }
  sub(b: C): C { return new C(this.re - b.re, this.im - b.im) }
  mul(b: C): C { return new C(this.re*b.re - this.im*b.im, this.re*b.im + this.im*b.re) }
  div(b: C): C {
  ...
  conj(): C { return new C(this.re, -this.im) }
  ln(): C { return new C(Math.log(this.abs()), this.arg()) }
  pow(b: C): C {
    if (this.re === 0 && this.im === 0) return new C(0);
  ...
  eq(b: C): boolean { return Math.abs(this.re - b.re) < 1e-12 && Math.abs(this.im - b.im) < 1e-12 }
  show(): string {
    if (Math.abs(this.im) < 1e-12) return niceNum(this.re);
    if (Math.abs(this.re) < 1e-12) return niceNum(this.im) + 'i';
    const s = this.im < 0 ? ' - ' : ' + ';
    return niceNum(this.re) + s + niceNum(Math.abs(this.im)) + 'i';
  }
  static from(n: number): C { return new C(n, 0) }
}
...
type AST
  = { n: 'num'; v: number }
  | { n: 'un'; op: string; a: AST }
  | { n: 'bin'; op: string; a: AST; b: AST }
  | { n: 'call'; fn: string; a: AST }
  | { n: 'var' };
...
function tokenize(s: string): Token[] {
  ...
  while (i < s.length) {
    ...
    if ('+-*/^()'.includes(s[i])) {
      if (s[i] === '(') t.push({ t: 'lp' });
      else if (s[i] === ')') t.push({ t: 'rp' });
      else t.push({ t: 'op', v: s[i] });
      i++; continue;
    }
    if (s[i] === '.' || (s[i] >= '0' && s[i] <= '9')) {
      let v = ''; while (i < s.length && (s[i] >= '0' && s[i] <= '9' || s[i] === '.')) { v += s[i]; i++ }
      t.push({ t: 'num', v: parseFloat(v) }); continue;
    }
    if ((s[i] >= 'a' && s[i] <= 'z') || (s[i] >= 'A' && s[i] <= 'Z')) {
      let id = ''; while (i < s.length && ((s[i] >= 'a' && s[i] <= 'z') || (s[i] >= 'A' && s[i] <= 'Z') || (s[i] >= '0' && s[i] <= '9'))) { id += s[i]; i++ }
      if (id === 'pi') t.push({ t: 'num', v: Math.PI });
      else if (id === 'e') t.push({ t: 'num', v: Math.E });
      else if (id === 'i') t.push({ t: 'num', v: 0 }); // handled as complex later
      else t.push({ t: 'id', v: id }); continue;
    }
    return [];
  }
  return t;
...
function parse(s: string): AST | null {
  const toks = tokenize(s);
  if (toks.length === 0) return null;
  try { return parseExpr(toks, 0)[0] } catch { return null }
}
function parseExpr(toks: Token[], i: number): [AST, number] { return parseAddSub(toks, i); }
function parseAddSub(toks: Token[], i: number): [AST, number] {
  let [l, i2] = parseMulDiv(toks, i);
  while (i2 < toks.length) {
    const tk = toks[i2];
    if (tk.t !== 'op' || (tk.v !== '+' && tk.v !== '-')) break;
    const [r, i3] = parseMulDiv(toks, i2 + 1);
    l = { n: 'bin', op: tk.v, a: l, b: r }; i2 = i3;
  }
  return [l, i2];
}
function parseMulDiv(toks: Token[], i: number): [AST, number] {
  let [l, i2] = parsePower(toks, i);
  while (i2 < toks.length) {
    const tk = toks[i2];
    if (tk.t === 'op' && (tk.v === '*' || tk.v === '/')) {
      const [r, i3] = parsePower(toks, i2 + 1); l = { n: 'bin', op: tk.v, a: l, b: r }; i2 = i3;
    } else if (tk.t === 'num' || tk.t === 'id' || tk.t === 'lp') {
      const [r, i3] = parsePower(toks, i2); l = { n: 'bin', op: '*', a: l, b: r }; i2 = i3;
    } else break;
  }
  return [l, i2];
}
function parsePower(toks: Token[], i: number): [AST, number] {
  let [l, i2] = parseUnary(toks, i);
  if (i2 < toks.length) { const tk = toks[i2]; if (tk.t === 'op' && tk.v === '^') { const [r, i3] = parsePower(toks, i2 + 1); return [{ n: 'bin', op: '^', a: l, b: r }, i3]; } }
  return [l, i2];
}
function parseUnary(toks: Token[], i: number): [AST, number] {
  if (i < toks.length && toks[i].t === 'op' && toks[i].v === '-') { const [a, i2] = parseUnary(toks, i + 1); return [{ n: 'un', op: '-', a }, i2]; }
  if (i < toks.length && toks[i].t === 'op' && toks[i].v === '+') { return parseUnary(toks, i + 1); }
  return parseAtom(toks, i);
}
function parseAtom(toks: Token[], i: number): [AST, number] {
  if (i >= toks.length) return [{ n: 'num', v: 0 }, i];
  const tk = toks[i];
  if (tk.t === 'num') { return [{ n: 'num', v: tk.v }, i + 1]; }
  if (tk.t === 'id') {
    if (FUNCS.has(tk.v)) {
      if (i + 1 < toks.length && toks[i+1].t === 'lp') { const [a, i2] = parseParen(toks, i + 1); return [{ n: 'call', fn: tk.v, a }, i2]; }
      const [a, i2] = parseAtom(toks, i + 1); return [{ n: 'call', fn: tk.v, a }, i2];
    }
    if (tk.v === 'x') return [{ n: 'var' }, i + 1];
    return [{ n: 'num', v: 0 }, i + 1];
  }
  if (tk.t === 'lp') return parseParen(toks, i);
  return [{ n: 'num', v: 0 }, i + 1];
}
function parseParen(toks: Token[], i: number): [AST, number] {
  if (toks[i].t === 'lp') { const [a, i2] = parseExpr(toks, i + 1); if (i2 < toks.length && toks[i2].t === 'rp') return [a, i2 + 1]; return [a, i2]; }
  return parseAtom(toks, i);
}

// ─── Eval (complex) ──────────────────────────────────────
function evalC(ast: AST, x: C, ans: C): C {
  switch (ast.n) {
    case 'num': return new C(ast.v);
    case 'var': return x;
    case 'un': { const v = evalC(ast.a, x, ans); return ast.op === '-' ? v.mul(new C(-1)) : v; }
    case 'bin': {
    ...
    case 'call': {
      ...
      switch (ast.fn) {
        ...
        case 'tan':    { const s = evalC({ n: 'call', fn: 'sin', a: ast.a }, x, ans), c = evalC({ n: 'call', fn: 'cos', a: ast.a }, x, ans); return s.div(c); }
        ...
        case 'tanh':   { const s = evalC({ n: 'call', fn: 'sinh', a: ast.a }, x, ans), c = evalC({ n: 'call', fn: 'cosh', a: ast.a }, x, ans); return s.div(c); }
...
function evalReal(ast: AST, x: number): number { return evalC(ast, new C(x), new C(0)).re; }

// ─── Coefficient recovery (for solvers) ──────────────────
function coefLinear(ast: AST): [number, number] {
  const f0 = evalReal(ast, 0), f1 = evalReal(ast, 1);
  const a = f1 - f0, b = f0;
  return [a, b];
}
function coefQuadratic(ast: AST): [number, number, number] {
...
function coefCubic(ast: AST): [number, number, number, number] {
...
// ─── Solvers ──────────────────────────────────────────────
function solveLinear(ast: AST): string {
  const [a, b] = coefLinear(ast);
  if (Math.abs(a) < 1e-12) return Math.abs(b) < 1e-12 ? 'All real numbers' : 'No solution';
  return 'x = ' + niceNum(-b/a);
}
function solveQuadratic(ast: AST): string {
  const [a, b, c] = coefQuadratic(ast);
  if (Math.abs(a) < 1e-12) return solveLinear(ast);
...
function solveCubic(ast: AST): string {
  const [a, b, c, d] = coefCubic(ast);
  if (Math.abs(a) < 1e-12) return solveQuadratic(ast);
...
// ─── Numerical methods ───────────────────────────────────
function adaptSimpson(f: (x: number) => number, a: number, b: number): number {
  function simpson(lo: number, hi: number): number { const m = (lo+hi)/2; return (hi-lo)/6 * (f(lo) + 4*f(m) + f(hi)); }
  function go(lo: number, hi: number, whole: number, tol: number): number {
...
function derivative(f: (x: number) => number, x: number): number { const h = 1e-8; return (f(x+h) - f(x-h)) / (2*h); }
function derivative2(f: (x: number) => number, x: number): number { const h = 1e-5; return (f(x-h) - 2*f(x) + f(x+h)) / (h*h); }
function derivativeN(n: number, f: (x: number) => number, x: number): number {
  if (n === 0) return f(x);
  if (n === 1) return derivative(f, x);
  if (n === 2) return derivative2(f, x);
  const h = Math.max(1e-8, 1e-3 / n);
  return (derivativeN(n-1, f, x+h) - derivativeN(n-1, f, x-h)) / (2*h);
}
function limit(f: (x: number) => number, a: number): number { return f(a + 1e-10); }
function taylorSeries(f: (x: number) => number, a: number, n: number, atX: number): number {
  let sum = 0, fact = 1;
  for (let i = 0; i <= n; i++) {
...
// ─── SVG generation ──────────────────────────────────────
function generateGraphSVG(ast: AST, xMin: number, xMax: number, yMin?: number, yMax?: number): string {
  ...
  if (pts.length === 0) return '<svg width="600" height="400"><text x="10" y="20">No points</text></svg>';
  const ys = pts.map(p => p[1]);
  const yLo = yMin ?? Math.min(...ys) - (Math.max(...ys) - Math.min(...ys)) * 0.1;
  const yHi = yMax ?? Math.max(...ys) + (Math.max(...ys) - Math.min(...ys)) * 0.1;
  const mx = (x: number) => (x - xMin) / (xMax - xMin) * w;
  const my = (y: number) => h - (y - yLo) / (yHi - yLo) * h;
  const path = pts.map(([x, y], i) => (i === 0 ? 'M' : 'L') + mx(x).toFixed(2) + ',' + my(y).toFixed(2)).join(' ');
  const grid = (n: number, axis: 'x' | 'y') => {
    let s = '';
...
function generateFractalSVG(
  ...
  function smoothColor(mu: number): string {
    if (mu === 0) return '#000';
    const t = mu / maxIter;
    const hue = 360 * 4 * t;
  ...
  switch (ast.n) {
    ...
    case 'bin': {
      ...
      switch (ast.op) {
        ...
        case '^':
          if (b.n === 'num') return { n: 'bin', op: '*', a: { n: 'bin', op: '*', a: { n: 'num', v: b.v }, b: { n: 'bin', op: '^', a, b: { n: 'num', v: b.v - 1 } } }, b: derivAST(a) };
...
// ─── Show AST as string ──────────────────────────────────
function showAST(ast: AST, prec: number = 0): string {
  const needParen = (p: number) => prec > p;
  switch (ast.n) {
...
function antiderivAST(ast: AST): AST | null {
  if (ast.n === 'num') return { n: 'bin', op: '*', a: ast, b: { n: 'var' } };
  if (ast.n === 'var') return { n: 'bin', op: '/', a: { n: 'bin', op: '^', a: { n: 'var' }, b: { n: 'num', v: 2 } }, b: { n: 'num', v: 2 } };
  if (ast.n === 'bin' && ast.op === '+') {
    const l = antiderivAST(ast.a), r = antiderivAST(ast.b);
    if (l && r) return { n: 'bin', op: '+', a: l, b: r };
  }
  if (ast.n === 'bin' && ast.op === '-') {
    const l = antiderivAST(ast.a), r = antiderivAST(ast.b);
    if (l && r) return { n: 'bin', op: '-', a: l, b: r };
  ...
  if (ast.n === 'call') {
    if (ast.fn === 'sin') return { n: 'un', op: '-', a: { n: 'call', fn: 'cos', a: ast.a } };
    if (ast.fn === 'cos') return { n: 'call', fn: 'sin', a: ast.a };
    if (ast.fn === 'exp') return { n: 'call', fn: 'exp', a: ast.a };
    if (ast.fn === 'ln')  return { n: 'bin', op: '-', a: { n: 'bin', op: '*', a: { n: 'var' }, b: ast }, b: { n: 'var' } };
  }
  return null;
}
...
async function evalExpr(expr: string) {
  ...
  if (!expr.trim()) return;
  if (expr.startsWith(':')) { await handleCommand(expr); return; }
...
async function handleCommand(cmd: string) {
  ...
  switch (command) {
    ...
    case 'about': addLine('Integra v1.0 — Web Calculator\nComplex numbers, calculus, graphing, fractals\nBuilt with TypeScript', 'info'); break;
    case 'clear': output.innerHTML = ''; break;
    case 'quit': addLine('Close this tab to quit.', 'info'); break;
    ...
    case 'deriv': {
      ...
      if (!ast) { addLine('Error: cannot parse expression', 'error'); break; }
    ...
    case 'deriv2': {
      const m = rest.match(/^(.*?)\s+at\s+(.+)$/);
      if (!m) { addLine('Usage: :deriv2 <expr> at <x>', 'error'); break; }
      const ast = parse(m[1]);
      if (!ast) { addLine('Error: cannot parse', 'error'); break; }
      const x = parseFloat(m[2]);
      if (!isFinite(x)) { addLine('Error: invalid x', 'error'); break; }
      const r = derivative2((xv: number) => evalReal(ast, xv), x);
    ...
    case 'derivn': {
      ...
      if (!m) { addLine('Usage: :derivn <expr> order <n> at <x>', 'error'); break; }
      const ast = parse(m[1]);
      if (!ast) { addLine('Error: cannot parse', 'error'); break; }
      const n = parseInt(m[2]), x = parseFloat(m[3]);
      if (!isFinite(n) || !isFinite(x)) { addLine('Error: invalid n or x', 'error'); break; }
      const r = derivativeN(n, (xv: number) => evalReal(ast, xv), x);
    ...
    case 'integral': {
      ...
      if (m) {
        ...
        if (!isFinite(a) || !isFinite(b)) { addLine('Error: invalid bounds', 'error'); break; }
        const r = adaptSimpson((xv: number) => evalReal(ast, xv), a, b);
    ...
    case 'limit': {
      ...
      if (!m) { addLine('Usage: :limit <expr> as <x>', 'error'); break; }
      const ast = parse(m[1]);
      if (!ast) { addLine('Error: cannot parse', 'error'); break; }
      const x = parseFloat(m[2]);
      if (!isFinite(x)) { addLine('Error: invalid x', 'error'); break; }
      const r = limit((xv: number) => evalReal(ast, xv), x);
    ...
    case 'taylor': {
      ...
      if (!m) { addLine('Usage: :taylor <expr> at <a> order <n>', 'error'); break; }
      const ast = parse(m[1]);
      if (!ast) { addLine('Error: cannot parse', 'error'); break; }
      const a = parseFloat(m[2]), n = parseInt(m[3]);
      if (!isFinite(a) || !isFinite(n)) { addLine('Error: invalid a or n', 'error'); break; }
      const r = taylorSeries((xv: number) => evalReal(ast, xv), a, Math.min(n, 10), 0);
    ...
    case 'solvec': {
      ...
      const ast = parse(expr);
      if (!ast) { addLine('Error: cannot parse', 'error'); break; }
      const solvers: Record<string, (e: AST) => string> = { solve: solveLinear, solveq: solveQuadratic, solvec: solveCubic };
    ...
    case 'explain': {
      ...
      if (!m) { addLine('Usage: :explain deriv|integral|solve <expr>', 'error'); break; }
      const ast = parse(m[2]);
      if (!ast) { addLine('Error: cannot parse', 'error'); break; }
      if (m[1] === 'deriv') {
        const d = derivAST(ast);
    ...
    case 'graph': {
      ...
      if (!isFinite(xMin) || !isFinite(xMax)) { addLine('Error: invalid bounds', 'error'); break; }
