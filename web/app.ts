// ─── Types ───────────────────────────────────────────────
interface EvalResponse     { result: string; expr: string }
interface DerivResponse    { deriv: string; result: string; expr: string; at: string }
interface SolveResponse    { solution: string; expr: string }
interface IntegralResponse { result: string; expr: string; from: string; to: string }
interface ExplainResponse  { text: string; type: string }
interface AboutResponse    { text: string }
...
function tokenize(s: string): Token[] {
  ...
  while (i < s.length) {
    ...
    if (s[i] === '.' || (s[i] >= '0' && s[i] <= '9')) {
      let v = '';
      while (i < s.length && (s[i] >= '0' && s[i] <= '9' || s[i] === '.')) { v += s[i]; i++ }
      t.push({ t: 'num', v: parseFloat(v) });
      continue;
    }
    if ((s[i] >= 'a' && s[i] <= 'z') || (s[i] >= 'A' && s[i] <= 'Z')) {
      let id = '';
      while (i < s.length && ((s[i] >= 'a' && s[i] <= 'z') || (s[i] >= 'A' && s[i] <= 'Z') || (s[i] >= '0' && s[i] <= '9'))) { id += s[i]; i++ }
      if (id === 'pi') t.push({ t: 'num', v: Math.PI });
      else if (id === 'e') t.push({ t: 'num', v: Math.E });
      else if (id === 'i' || id === 'phi' || id === 'tau') t.push({ t: 'num', v: 0 });
      else t.push({ t: 'id', v: id });
      continue;
    }
    return []; // error
  }
...
function parseParen(toks: Token[], i: number): [AST, number] {
  if (toks[i].t === 'lp') {
    ...
    if (i2 < toks.length && toks[i2].t === 'rp') return [a, i2 + 1];
    return [a, i2];
  }
  return parseAtom(toks, i);
}

function evalAST(ast: AST, x: number): number {
  switch (ast.n) {
...
function clientEval(expr: string): string | null {
  try {
...
async function tryAPIFallback<T>(url: string): Promise<T | null> {
  try {
...
async function handleCommand(cmd: string): Promise<void> {
  ...
  switch (command) {
    ...
    case 'about': {
      const data = await tryAPIFallback<AboutResponse>(API_BASE + '/about');
      if (data) addLine(data.text, 'info');
      else addLine('Integra v1.0 — Web Calculator', 'info');
      break;
    }
    ...
    case 'deriv': {
      ...
      try {
        ...
        if (data.result) addLine("f'(" + atStr + ') = ' + data.result, 'result');
      } catch {
    ...
    case 'deriv2': {
      ...
      try {
        const data = await fetchJSON<EvalResponse>(API_BASE + '/deriv2?expr=' + encodeURIComponent(m[1]) + '&at=' + encodeURIComponent(m[2]));
        addLine("f''(" + m[2] + ') = ' + data.result, 'result');
      } catch { addLine('Requires a backend server.', 'error'); }
      break;
    }
    ...
    case 'derivn': {
      ...
      try {
        const data = await fetchJSON<EvalResponse>(API_BASE + '/derivn?expr=' + encodeURIComponent(m[1]) + '&order=' + encodeURIComponent(m[2]) + '&at=' + encodeURIComponent(m[3]));
    ...
    case 'integral': {
      ...
      if (m) {
        try {
          const data = await fetchJSON<IntegralResponse>(API_BASE + '/integral?expr=' + encodeURIComponent(m[1]) + '&from=' + encodeURIComponent(m[2]) + '&to=' + encodeURIComponent(m[3]));
          addLine('\u222B f(x) dx = ' + data.result, 'result');
        } catch { addLine('Requires a backend server.', 'error'); }
      } else {
        try {
          const data = await fetchJSON<EvalResponse>(API_BASE + '/antideriv?expr=' + encodeURIComponent(rest));
          addLine('\u222B f(x) dx = ' + data.result, 'result');
        } catch { addLine('Requires a backend server.', 'error'); }
      }
      break;
    ...
    case 'limit': {
      ...
      try {
        const data = await fetchJSON<EvalResponse>(API_BASE + '/limit?expr=' + encodeURIComponent(m[1]) + '&as=' + encodeURIComponent(m[2]));
    ...
    case 'taylor': {
      ...
      try {
        const data = await fetchJSON<EvalResponse>(API_BASE + '/taylor?expr=' + encodeURIComponent(m[1]) + '&at=' + encodeURIComponent(m[2]) + '&order=' + encodeURIComponent(m[3]));
    ...
    case 'solvec': {
      ...
      try {
        const data = await fetchJSON<SolveResponse>(API_BASE + '/' + ep + '?expr=' + encodeURIComponent(rest));
    ...
    case 'explain': {
      ...
      try {
        const data = await fetchJSON<ExplainResponse>(API_BASE + '/explain?type=' + encodeURIComponent(m[1]) + '&expr=' + encodeURIComponent(m[2]));
    ...
    case 'burningship': {
      ...
      try {
        ...
        if (command === 'julia') {
          ...
          if (args[5]) url += '&xMin=' + args[5];
          if (args[6]) url += '&xMax=' + args[6];
          if (args[7]) url += '&yMin=' + args[7];
          if (args[8]) url += '&yMax=' + args[8];
        } else {
          url = API_BASE + '/' + command + '?width=' + (args[0] || '400') + '&height=' + (args[1] || '400') + '&iter=' + (args[2] || '100');
          if (args[3]) url += '&xMin=' + args[3];
          if (args[4]) url += '&xMax=' + args[4];
          if (args[5]) url += '&yMin=' + args[5];
          if (args[6]) url += '&yMax=' + args[6];
        }
        const svg = await fetchText(url);
        addHTML('<div class="svg-container">' + svg + '</div>');
      } catch { addLine('Requires a backend server.', 'error'); }
